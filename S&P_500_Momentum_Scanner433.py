...
# (Kept for compatibility, though not used now that Exchange is suppressed)
...
def map_to_yfinance_symbol(symbol: str, exchange: str) -> str:
    # No longer used — we rely on Symbol directly as a YF symbol
    return symbol
...
def get_ticker_data(_ticker, _exchange_unused, yf_symbol):
    """Exchange suppressed — keep signature for minimal change; ignore exchange in payload."""
    try:
...
def main():
    ...
    if uploaded_file is not None:
        ...
        # With Exchange suppressed, assume Symbol is already a valid yfinance symbol
        df["YF_Symbol"] = df["Symbol"]
    else:
        st.warning("Please upload a .xlsx file with your tickers.")
        return

    # Sidebar filters (keep existing ones; Exchange filter removed)
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = [
            executor.submit(get_ticker_data, row["Symbol"], None, row["YF_Symbol"])
