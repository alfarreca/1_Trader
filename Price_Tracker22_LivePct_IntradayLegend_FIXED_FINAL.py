...
if uploaded_file:
    ...
    if sheet_choice:
        ...
        if not all(col in df.columns for col in ["Symbol", "Exchange"]):
            ...
            if all_data:
                # --- Intraday % Change vs Yesterday's Close ---
                live_pct_change = {}
                for sym in all_data:
                    try:
                        df = yf.download(sym, period='2d', interval='1d', progress=False)
                        if df.shape[0] >= 2 and 'Close' in df:
                            yest_close = df['Close'].iloc[-2]
                            today_price = df['Close'].iloc[-1]
                        else:
                            ticker = yf.Ticker(sym)
                            yest_close = ticker.info.get('previousClose')
                            today_price = ticker.info.get('regularMarketPrice')
                        if pd.notna(yest_close) and pd.notna(today_price) and yest_close != 0:
                            change = (today_price - yest_close) / yest_close * 100
                            live_pct_change[sym] = round(change, 2)
                        else:
                            live_pct_change[sym] = 0.0
